...
1) Proposal（提案阶段，禁止写代码）
   ...
   - 照抄入口（Claude Code：显式点名 Skills，保证触发；只需填“我的需求”）：
     ...
     5) （可选）当"对外行为/契约/数据不变量"变化时，使用 `devbooks-spec-contract`：
        - 写：`openspec/changes/<change-id>/specs/<capability>/spec.md` + `design.md` Contract 章节
...
其他 Skill 脚本：
- 隐式变更检测（devbooks-spec-contract）：`"$HOME/.claude/skills/devbooks-spec-contract/scripts/implicit-change-detect.sh" --project-root "$(pwd)"`
