...
- 护栏检查：`"$DEVBOOKS_SCRIPTS/guardrail-check.sh" <change-id> --project-root "$(pwd)" --change-root <change-root>`
...
- `devbooks-impact-analysis` / `devbooks-design-doc` / `devbooks-spec-delta` / `devbooks-implementation-plan`
- `devbooks-test-owner` / `devbooks-coder` / `devbooks-code-review` / `devbooks-spec-gardener`
- `devbooks-contract-data` / `devbooks-c4-map` / `devbooks-design-backport`
