...
4) 如果用户要求你运行 `openspec validate ...`（或 `openspec-cn validate ...`）：
   ...
   - 你只能：把报错摘要写进 `proposal.md` 的 Debate Packet 或 Open Questions，并给出下一步应由哪个角色/Skill 处理（例如 `devbooks-design-doc` / `devbooks-spec-delta` / `devbooks-implementation-plan` / `devbooks-test-owner`）。
