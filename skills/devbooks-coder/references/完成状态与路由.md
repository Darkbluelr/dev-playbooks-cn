...
| ⚠️ | IMPLEMENTATION_DONE_WITH_DEVIATION | 快轨绿，deviation-log 有未回写记录 | `devbooks-design-doc` |
...
| IMPLEMENTATION_DONE_WITH_DEVIATION | `devbooks-design-doc` | 先回写设计决策（归档阶段由 Archiver 自动回写；需要提前回写时使用） |
...
- 如有偏离，必须先完成设计回写（使用 `devbooks-design-doc`；归档阶段由 Archiver 自动回写）再继续
