...
2. **大变更**：回写设计决策（如需在归档前回写，使用 `devbooks-design-doc`；归档阶段 Archiver 也会自动回写）
3. **可测变更**：使用 `devbooks-spec-contract` 补充契约测试（Contract Tests）

## 硬约束

1. 隐式变更检测是 `devbooks-spec-contract` 的**扩展功能**，不单独成为新 Skill
