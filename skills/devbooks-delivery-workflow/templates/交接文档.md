# 角色交接记录 / Role Handoff Record

## 交接信息 / Handoff Metadata

- **变更ID**: <change-id>
- **交出角色**: <Test Owner / Design Owner / Planner / ...>
- **接收角色**: <Coder / Reviewer / ...>
- **交接时间**: <YYYY-MM-DD HH:MM>
- **对话/实例ID**: <session-id>

## 交接内容 / Handoff Content

### 已完成产物 / Completed Deliverables

- [ ] `design.md` - 设计文档已产出并通过评审
- [ ] `tasks.md` - 编码计划已产出
- [ ] `verification.md` - 验收测试已定义
- [ ] `tests/**` - 测试代码已实现
- [ ] Red 基线已记录到 `evidence/red-baseline/`

### 上下文信息 / Context Information

> 请填写接收角色需要了解的关键信息

- **当前状态**: <简述当前进展>
- **关键决策**: <需要了解的重要决策>
- **已知问题**: <可能遇到的问题或风险>
- **参考文档**: `RUNBOOK.md#Context Capsule`（risk_level=medium|high 或 request_kind=epic|governance 或 intervention_level=team|org 时 MUST）；<其他需要阅读的文档>

### 必跑锚点 / Must-run Anchors

> 必须列出接收方“照抄就能跑”的验证锚点；与 `RUNBOOK.md#Context Capsule` 的“必跑验证锚点”对齐。
> 若确实无额外锚点，写 `N/A`（不要留空）。

- <C-xxx> <invocation_ref / command>

### 弱连接义务 / Weak-Link Obligations

> 必须列出本次变更涉及的“代码外同步点/弱连接义务”（例如 docs/config/release/ops/migration）。
> 建议写成 `completion.contract.yaml::obligations[]` 的 `O-###`（tags 含 `weak_link`），并指向其证据落点（`evidence/**`）。
> 若无弱连接义务，写 `N/A`（不要留空）。

- <O-xxx> <what to sync> → evidence: <evidence/path>

### 续做点 / Continuation Points

> 请列出接收角色需要完成的工作
> 若已无续做点，写 `N/A`（不要留空；不要保留 `<...>` 占位符）。

1. <待完成项 1>
2. <待完成项 2>

## 确认签名 / Confirmation Signatures

> 交接完成后，双方在此签名确认

- [ ] **交出方确认**: 我确认已完成上述产物，信息准确完整
- [ ] **接收方确认**: 我确认已收到交接内容，理解当前状态和待完成工作

---

## 附注 / Notes

> 可选：其他需要记录的信息

<填写附加说明或留空>
