...
| ⚠️ | COMPLETED_WITH_DEVIATION | 完成但有偏离/新增 | 先回写设计决策（如需在归档前回写，用 `devbooks-design-doc`；否则归档时由 Archiver 自动回写），再下一步 |
...
## 与设计回写（design writeback）的交互

说明：归档阶段 `devbooks-archiver` 会自动执行设计回写；如需在归档前回写，请使用 `devbooks-design-doc`。旧名称（已废弃）：`devbooks-design-backport`。

### 触发条件

当 deviation-log.md 中有**未回写记录**（已回写=❌）时，必须先完成设计回写。
