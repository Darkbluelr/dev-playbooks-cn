...
> 归档不是“把目录移走”，而是把变更的**证据链**与**真理工件**收敛到可审计状态。
...
说明：
- 默认：检查结果用于记录与提示（便于修复悬挂引用/缺失路径等问题）。
- 当 G6 判定 docs 一致性为必需证据项时（deliverables 涉及 `README.md/docs/**/templates/**`，或存在 `weak_link+docs` 的 `severity=must` 义务）：
  - `archive-decider.sh` 在裁判时会尝试自动生成 `evidence/gates/docs-consistency.report.json`（调用 `docs-consistency-check.sh`），以减少“忘了跑”的人为漂移。
  - 若生成后仍缺失或 `status!=pass`，将被判定为 fail，从而阻塞归档。
  - 仍建议在归档前先手动运行一次以获得更快反馈（尤其当文档引用变更较多时）。
...
> 可选但推荐：若项目启用了项目级 SSOT（`<truth-root>/ssot/requirements.index.yaml`），归档完成后可刷新派生进度视图（可丢弃缓存）：
>
> ```bash
> requirements-ledger-derive.sh --project-root <project-root> --change-root <change-root> --truth-root <truth-root>
> ```
>
> 默认输出：`<truth-root>/ssot/requirements.ledger.yaml`
