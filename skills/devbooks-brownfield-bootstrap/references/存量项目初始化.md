...
- **证据优先**：每条规格都要能指向代码/测试/日志/运行方式。
- **TBD 策略**：`spec.md` 禁止 TBD/TODO（闸门会拒绝）；未知项只能放入 `verification.md` 的 Open Questions 或 MANUAL 验证任务，并绑定具体验证计划。
...
注意：
- 发现不确定：**不要在 spec.md 中写 TBD**；把验证动作加入 `verification.md` 的 Open Questions 或 MANUAL 验证任务（附验证计划）。
