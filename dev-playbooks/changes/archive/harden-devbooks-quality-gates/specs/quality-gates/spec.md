...
- **证据**: `skills/devbooks-delivery-workflow/scripts/change-check.sh`
...
### Requirement: REQ-QG-MOD-001 模式参数扩展

**描述**：系统 MUST 增强原有 `change-check.sh` 的 `--mode` 参数行为。

**优先级**：P0（必须）

**变更内容**：
- `--mode proposal`：保持不变
- `--mode apply`：新增 `--role` 参数支持
- `--mode archive`：新增 Green 证据检查
- `--mode strict`：新增任务完成率检查、P0 跳过审批检查

#### Scenario: SC-QG-MOD-001 使用新增模式参数
...
|-------|-------------|----------|
| AC-001 | REQ-QG-001 | SC-QG-001-01, SC-QG-001-02 |
| AC-002 | REQ-QG-002 | SC-QG-002-01, SC-QG-002-02 |
| AC-003 | REQ-QG-003 | SC-QG-003-01, SC-QG-003-02 |
| AC-005 | REQ-QG-004 | SC-QG-004-01, SC-QG-004-02 |
| AC-007 | REQ-QG-005 | SC-QG-005-01, SC-QG-005-02 |
