...
last_change: 20260122-0827-enhance-docs-consistency
archive_verified: 2026-01-23
---
...
| `devbooks-docs-consistency` | 维护 | 文档一致性检查（改名自 devbooks-docs-sync） | - |

> 变更说明（来源：20260122-0827-enhance-docs-consistency）：
> - `devbooks-docs-sync` 改名为 `devbooks-docs-consistency`
> - 新增子组件：RulesEngine（规则引擎）、Scanner（扫描器）、CompletenessChecker（完备性检查器）、DocClassifier（文档分类器）、StyleChecker（风格检查器）
...
### 2026-01-23: 文档一致性工具优化（20260122-0827-enhance-docs-consistency）

**Component 变更**：
- [改名] `devbooks-docs-sync` → `devbooks-docs-consistency`
- [新增] RulesEngine 组件（规则引擎）
- [新增] Scanner 组件（增量/全量扫描）
- [新增] CompletenessChecker 组件（完备性检查器）
- [新增] DocClassifier 组件（文档分类器）
- [新增] StyleChecker 组件（风格检查器）

**依赖变更**：
- [修改] `devbooks-archiver` → `devbooks-docs-consistency`（调用改名后的 skill）
- [新增] `devbooks-brownfield-bootstrap` → `devbooks-docs-consistency`（初始化时生成元数据）
- [新增] `devbooks-docs-consistency` → Git（增量扫描依赖）
- [删除] `devbooks-docs-consistency` → MCP（删除 MCP 增强功能）

**分层约束影响**：无变更，依赖方向正确，无循环依赖。
