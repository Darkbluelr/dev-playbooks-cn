...
- 范围：仓库顶层目录与关键脚本的引用关系
- 来源：`scripts/install-skills.sh`，`README.md`，`docs/embedding-quickstart.md`
...
2. `scripts/install-skills.sh` → `templates/claude-commands/`
   - 说明：可选安装命令入口模板到 `$CODEX_HOME/prompts/devbooks/`
   - 证据：`scripts/install-skills.sh`
3. `docs/` → `tools/`
