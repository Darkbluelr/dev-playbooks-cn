...
- Bash 脚本：安装、Hook、工具脚本
  - 证据：`scripts/install-skills.sh`，`scripts/install-dependencies.sh`
...
- `templates/claude-commands/`：Slash Commands 模板
- `scripts/`：安装与辅助脚本
...
- `README.md` 引导执行 `scripts/install-skills.sh`
...
- DevBooks 协议集成与配置模板
  - 证据：`templates/.devbooks/config.yaml`，`templates/dev-playbooks/project.md`
- 协议发现脚本输出格式
